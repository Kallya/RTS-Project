...
public class PlayerCommandInput : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Alpha1))
            _commandProcessor.QueueCommand(new ChangePOVCommand(gameObject, 1));
        
        if (Input.GetKeyDown(KeyCode.Alpha2))
            _commandProcessor.QueueCommand(new ChangePOVCommand(gameObject, 2));
        
        if (Input.GetKeyDown(KeyCode.Alpha3))
            _commandProcessor.QueueCommand(new ChangePOVCommand(gameObject, 3));

        if (Input.GetKeyDown(KeyCode.Alpha4))
            _commandProcessor.QueueCommand(new ChangePOVCommand(gameObject, 4));
